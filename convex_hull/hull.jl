...
# ╔═╡ 59b13165-6e52-4099-9afa-e0961413fbba
using Images, TestImages, ImageDraw, LinearAlgebra

# ╔═╡ 9a23c81c-d9e3-4914-b5e2-e59f138a0ad4
img = Array{RGB{Normed{UInt8,8}},2}(undef, (100, 300));

# ╔═╡ 30f25b5d-2da5-4470-a075-a3db86457a98
function rand_points(img::Arr, k::Int, off::CartesianIndex{2}=CartesianIndex(0,0)) where Arr <: AbstractArray{T,2} where T <: Color
	(x, y) = size(img);
	xp = rand(1:1:x, k);
	yp = rand(1:1:y, k);
	
	rpoints = map(x -> CartesianIndex(x...), eachrow(hcat(xp, yp)))
...
# ╔═╡ 5c88fee4-f121-4604-9723-7396b735e1c1
begin
	img .= RGB{Normed{UInt8,8}}(1, 1, 1);
	# points = rand_points(view(img, 25:75, 25:275), 100, CartesianIndex(25, 25));
	points = rand_points(img, 100);
	img
end

# ╔═╡ 10c637c2-c7eb-40fc-adb6-7026f4b61251
function draw_lines(Arr::Array{T, 2}, points) where T
	img = copy(Arr)
	hull = Vector{LineSegment}()
	for i = 1:length(points)-1
		push!(hull, LineSegment(points[i], points[i+1]))
	end
	
	draw!(img, hull, T(0, 0, 0))
	return img
end

# ╔═╡ deb136b3-9978-4518-8357-efb33e6913f1
function right_turn(a::T, b::T, c::T) where T <: CartesianIndex
	v1 = b-a;
	v2 = c-b;
...
# ╔═╡ 5fddea3f-b3b9-4ecd-b787-9d5bc55acd07
function convex_hull(points::Vector{CartesianIndex{2}}, img)
	# setup
	ptype = CartesianIndex{2}
	
	n = length(points)
	sort!(points)
	Lu = Vector{CartesianIndex{2}}()
	Ll = Vector{CartesianIndex{2}}()
	gif = Array{RGB{Normed{UInt8,8}},3}(undef, (size(img)..., 1))
	
	# Algo
	push!(Lu, points[1])
	push!(Lu, points[2])
	gif[:, :, 1] = draw_lines(img, Lu)
	
	for i = 3:n
		while length(Lu) >= 2 && (right_turn(Lu[end-1], Lu[end], points[i]) <= 0)
			pop!(Lu)
			gif = cat(gif, draw_lines(img, Lu), dims=3);
		end
		push!(Lu, points[i])
		gif = cat(gif, draw_lines(img, Lu), dims=3);
	end
	...
	push!(Ll, points[end])
	push!(Ll, points[end-1])
	gif = cat(gif, draw_lines(lu_img, Ll), dims=3)
	
	for i = n-2:-1:1
		while length(Ll) >= 2 && (right_turn(Ll[end-1], Ll[end], points[i]) <= 0)
			pop!(Ll)
			gif = cat(gif, draw_lines(lu_img, Ll), dims=3)
		end
		push!(Ll, points[i])
	...
	last(Lu) == first(Ll) && pop!(Lu)
	
	append!(Lu, Ll);
	return Lu, gif
end

# ╔═╡ 9ac98118-ff34-45b5-996f-cd879072d047
hull, gif = convex_hull(points, img);

# ╔═╡ c116a846-884c-46d7-b6cd-a995ba97cbc7
save("test.gif", gif, fps=15)
